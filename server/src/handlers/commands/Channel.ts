import { ApplicationCommandDataResolvable, CategoryChannel, Collection, CommandInteraction, GuildChannel, GuildMember, PermissionFlags, PermissionOverwriteOptions, PermissionOverwrites, Permissions, Role, Snowflake, TextChannel } from "discord.js";
...
export class ChannelCommand implements IHandler {
  ...
  async archiveChannel(payload: CommandInteraction): Promise<any> {
    const config = await (await MongoCollection.getCollection('assets')).collection.findOne({ type: 'BotConfig', guildId: payload.guildId }) as WithId<BotConfig>;
    const channel = (payload.options.getChannel('channel') ?? payload.channel) as GuildChannel;
